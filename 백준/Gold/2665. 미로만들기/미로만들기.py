...
이렇게 하는 이유는 매번 우선순위 큐에서 원소를 뽑을 때마다 
벽을 바꾼 횟수가 최소인 지점부터 뽑을 수 있고, (=> 횟수를 우선순위로! 맨 앞에)
그 지점을 기준으로 상하좌우를 탐색해 나가기 때문입니다.
그러면 결과적으로 벽을 바꾼 횟수가 최소이면서, 도착점까지 갈 수 있겠죠.
'''
# 함수없이 구현
import sys
import heapq
...
pq = []  # 우선순위 큐
heapq.heappush(pq, (0, 0, 0))  # cnt(검은 방 -> 흰 방), 시작 방
while pq:   # pq가 빌 때까지
